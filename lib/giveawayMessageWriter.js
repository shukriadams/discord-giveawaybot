...
module.exports = {
    ...
    sendWinnerMessages : async function(client, giveaway, winner){
        ...
        let winnerMessage = `Congratulations, you just won ${giveaway.gameName}, courtesy of ${giveaway.ownerName} (<@${giveaway.ownerId}>). `;

        if (giveaway.code)
            winnerMessage += `Your game key is ${giveaway.code}, be sure to thank ${giveaway.ownerName} (<@${giveaway.ownerId}>) for the giveaway!`;
        else
